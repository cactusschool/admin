...
import com.ashish.cactus.school.admin.services.ModulesService;
...
public class ModuleController {
	...
	@ApiOperation(value="Get list of supported modules from database",
				notes="Get list of supported modules from database",
				response=AdminOutput.class
			)
	
	@ApiResponses (value = {
							@ApiResponse(code=200, message="Successfully retrieved the list of available modules"),
							@ApiResponse(code=400, message="Bad response")
						}
					)
	@RequestMapping(value="/modules", method=RequestMethod.GET)
	public AdminOutput getAllModules(
		...
		try {
			modulesService.getAllModules(adminOutput);
		} catch (Exception e) {
